...
import Scfuzzer
import Scfuzzer.Config
import Scfuzzer.Types.Buffer (forceBuf)
import Scfuzzer.Types.Campaign
import Scfuzzer.Types.Config
import Scfuzzer.Types.Solidity
import Scfuzzer.Types.Test (TestMode, ScfuzzerTest(..))
import Scfuzzer.Test (validateTestMode)
import Scfuzzer.Campaign (isSuccessful)
import Scfuzzer.UI
import Scfuzzer.Output.Source
import Scfuzzer.Output.Corpus
import Scfuzzer.RPC qualified as RPC
import Scfuzzer.Solidity (compileContracts, selectBuildOutput)
...
data Options = Options
  ...
  , cliSender           :: [Addr]
  , cliSeed             :: Maybe Int
  , cliKhulnasoftArgs       :: Maybe String
...
options = Options
  ...
  <*> optional (option str $ long "khulnasoft-args"
    <> metavar "ARGS"
    <> help "Additional arguments to use in khulnasoft-compile for the compilation of the contract to test.")
...
overrideConfig config Options{..} = do
  ...
  where
    ...
    overrideSolConf solConf = solConf
      ...
      , khulnasoftArgs = maybe solConf.khulnasoftArgs words cliKhulnasoftArgs
