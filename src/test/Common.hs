...
import Scfuzzer (prepareContract)
import Scfuzzer.Config (parseConfig, defaultConfig)
import Scfuzzer.Campaign (runWorker)
import Scfuzzer.Solidity (loadSolTests, compileContracts, selectBuildOutput)
import Scfuzzer.Test (checkETest)
import Scfuzzer.Types (Gas)
import Scfuzzer.Types.Config (Env(..), EConfig(..), EConfigWithUsage(..))
import Scfuzzer.Types.Campaign
import Scfuzzer.Types.Signature (ContractName)
import Scfuzzer.Types.Solidity (SolConf(..))
import Scfuzzer.Types.Test
import Scfuzzer.Types.Tx (Tx(..), TxCall(..), call)
...
getResult :: Text -> [ScfuzzerTest] -> Maybe ScfuzzerTest
getResult n tests =
