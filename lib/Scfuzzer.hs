...
-- * A list of Scfuzzer tests to check
...
prepareContract env contracts solFiles specifiedContract seed = do
  ...
  let
    -- load tests
    scfuzzerTests = createTests solConf.testMode
  ...
  writeIORef env.testsRef scfuzzerTests
  pure (vm, world, dict)
