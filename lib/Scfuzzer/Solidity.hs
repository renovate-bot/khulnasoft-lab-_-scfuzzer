module Scfuzzer.Solidity where
...
import Scfuzzer.Deploy (deployContracts, deployBytecodes)
import Scfuzzer.Etheno (loadEthenoBatch)
import Scfuzzer.Events (EventMap, extractEvents)
import Scfuzzer.Exec (execTx, initialVM)
import Scfuzzer.Processor
import Scfuzzer.Test (createTests, isAssertionMode, isPropertyMode, isDapptestMode)
import Scfuzzer.Types.Config (EConfig(..), Env(..))
import Scfuzzer.Types.Signature
  (ContractName, SolSignature, SignatureMap, getBytecodeMetadata)
import Scfuzzer.Types.Solidity
import Scfuzzer.Types.Test (ScfuzzerTest(..))
import Scfuzzer.Types.Tx
  ( basicTx, createTxWithValue, unlimitedGasPerBlock, initialTimestamp
  , initialBlockNumber )
import Scfuzzer.Types.World (World(..))
import Scfuzzer.Utility (measureIO)
...
compileContracts solConf fp = do
  path <- findExecutable "khulnasoft-compile" >>= \case
    Nothing -> throwM NoKhulnasoftCompile
    Just path -> pure path
    ...
    compileOne x = do
      ...
      (ec, out, err) <- measureIO solConf.quiet ("Compiling " <> x) $ do
        readCreateProcessWithExitCode
          (proc path $ (solConf.khulnasoftArgs ++ solargs) |> x) {std_err = stderr} ""
      case ec of
        ExitSuccess -> readSolcBatch "khulnasoft-export"
  ...
  removeJsonFiles "khulnasoft-export"
...
-- contract, or, if not provided, the first contract in the list, into a 'VM' usable for Scfuzzer
-- testing and extract an ABI and list of tests. Throws exceptions if anything returned doesn't look
-- usable for Scfuzzer. NOTE: Contract names passed to this function should be prefixed by the
...
-- its ABI (as 'SolSignature's), and the names of its Scfuzzer tests. NOTE: unlike 'loadSpecified',
-- contract names passed here don't need the file they occur in specified.
loadSolTests
  :: Env
  -> NonEmpty FilePath
  -> Maybe Text
  -> IO (VM, World, [ScfuzzerTest])
loadSolTests env fp name = do
  ...
  let
    ...
    scfuzzerTests = createTests solConf.testMode True testNames vm.state.contract funs
  pure (vm, world, scfuzzerTests)
