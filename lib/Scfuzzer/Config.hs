...
import Scfuzzer.Types.Tx (TxConf(TxConf), maxGasPerBlock, defaultTimeDelay, defaultBlockDelay)
import Scfuzzer.Types.Test (TestConf(..))
...
-- | The default config used by Scfuzzer (see the 'FromJSON' instance for values used).
defaultConfig :: EConfig
defaultConfig = either (error "Config parser got messed up :(") id $ Y.decodeEither' ""

-- | Try to parse an Scfuzzer config file, throw an error if we can't.
