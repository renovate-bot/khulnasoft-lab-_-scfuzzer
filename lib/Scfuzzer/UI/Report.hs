module Scfuzzer.UI.Report where
...
import Scfuzzer.ABI (GenDict(..), encodeSig)
import Scfuzzer.Events (Events)
import Scfuzzer.Pretty (ppTxCall)
import Scfuzzer.Types (Gas)
import Scfuzzer.Types.Campaign
import Scfuzzer.Types.Coverage (scoveragePoints)
import Scfuzzer.Types.Test (ScfuzzerTest(..), TestState(..), TestType(..))
import Scfuzzer.Types.Tx (Tx(..), TxCall(..), TxConf(..))
...
ppTests :: (MonadReader Env m) => [ScfuzzerTest] -> m String
ppTests tests = do
