...
module Scfuzzer.UI.Widgets where
...
import Paths_scfuzzer qualified (version)
...
import Scfuzzer.Utility (timePrefix)
...
campaignStatus uiState = do
  ...
  mainbox inner underneath = do
    ...
    pure $ hCenter . hLimit 120 $
      joinBorders $ borderWithLabel scfuzzerTitle $
  ...
  scfuzzerTitle =
    ...
    withAttr (attrName "title")
      (str $ "Scfuzzer " <> showVersion Paths_scfuzzer.version) <+>
    str " ]"
...
failedFirst :: ScfuzzerTest -> ScfuzzerTest -> Ordering
failedFirst t1 _ | didFail t1 = LT
                 | otherwise  = GT

testsWidget :: MonadReader Env m => [ScfuzzerTest] -> m (Widget Name)
testsWidget tests' =
  withClickableVScrollBars SBClick .
  withVScrollBars OnRight .
  withVScrollBarHandles .
...
testWidget :: MonadReader Env m => ScfuzzerTest -> m (Widget Name)
testWidget test =
