module Scfuzzer.Types.Test where
...
import Scfuzzer.Events (Events)
import Scfuzzer.Types (ExecException)
import Scfuzzer.Types.Signature (SolSignature)
import Scfuzzer.Types.Tx (Tx, TxResult)

-- | Test mode is parsed from a string
type TestMode = String

-- | Configuration for the creation of Scfuzzer tests.
...
-- | State of a particular Scfuzzer test. N.B.: 'Solved' means a falsifying
...
-- | An Scfuzzer test is represented with the following data record
data ScfuzzerTest = ScfuzzerTest
  { state      :: TestState
  , testType   :: TestType
  , value      :: TestValue
  , reproducer :: [Tx]
...
isOptimizationTest :: ScfuzzerTest -> Bool
isOptimizationTest ScfuzzerTest{testType = OptimizationTest _ _} = True
isOptimizationTest _ = False

isOpen :: ScfuzzerTest -> Bool
isOpen t = case t.state of
  Open -> True
  _    -> False

didFail :: ScfuzzerTest -> Bool
didFail t = case t.state of
  Large _ -> True
  Solved  -> True
  _       -> False

isPassed :: ScfuzzerTest -> Bool
