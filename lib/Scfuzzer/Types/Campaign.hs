module Scfuzzer.Types.Campaign where
...
import Scfuzzer.ABI (GenDict, emptyDict, encodeSig)
import Scfuzzer.Output.Source (CoverageFileType)
import Scfuzzer.Types
import Scfuzzer.Types.Coverage (CoverageMap)
import Scfuzzer.Types.Test (TestType (..), ScfuzzerTest(..))
import Scfuzzer.Types.Tx (Tx)

-- | Configuration for running an Scfuzzer 'Campaign'.
data CampaignConf = CampaignConf
  ...
  , knownCoverage   :: Maybe CoverageMap
    ...
    -- Scfuzzer won't collect coverage information (and will go faster)
  , seed            :: Maybe Int
...
data CampaignEvent
  = TestFalsified !ScfuzzerTest
  | TestOptimized !ScfuzzerTest
...
ppCampaignEvent = \case
  ...
  WorkerStopped (Crashed e) ->
    "Crashed:\n\n" <>
    e <>
    "\n\nPlease report it to https://github.com/khulnasoft-lab/scfuzzer/issues"
