module Scfuzzer.Types.Solidity where
...
-- | Things that can go wrong trying to load a Solidity file for Scfuzzer testing.
...
instance Show SolException where
  show = \case
    ...
    SolcReadFailure        -> "Could not read khulnasoft-export/combined_solc.json"
    ...
    NoKhulnasoftCompile        -> "khulnasoft-compile not installed or not found in PATH. To install it, run:\n   pip install khulnasoft-compile"
    ...
    OutdatedSolcVersion v  -> "Solc version " ++ toString v ++ " detected. Scfuzzer doesn't support versions of solc before " ++ toString minSupportedSolcVersion ++ ". Please use a newer version."


instance Exception SolException

-- | Configuration for loading Solidity for Scfuzzer testing.
data SolConf = SolConf
  ...
  , khulnasoftArgs      :: [String]         -- ^ Args to pass to khulnasoft
