...
import Scfuzzer.ABI
import Scfuzzer.Events (Events, extractEvents)
...
import Scfuzzer.Types.Test
import Scfuzzer.Types.Test qualified as Test
import Scfuzzer.Types.Tx (Tx, TxConf(..), basicTx, TxResult(..), getResult)

--- | Possible responses to a call to an Scfuzzer test: @true@, @false@, @REVERT@, and ???.
data CallRes = ResFalse | ResTrue | ResRevert | ResOther
  deriving (Eq, Show)

--- | Given a 'VMResult', classify it assuming it was the result of a call to an Scfuzzer test.
...
createTest :: TestType -> ScfuzzerTest
createTest m = ScfuzzerTest Open m v [] Stop []
  where v = case m of
