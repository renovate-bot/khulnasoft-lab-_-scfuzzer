...
module Scfuzzer.Output.JSON where
...
import Scfuzzer.ABI (ppAbiValue, GenDict(..))
import Scfuzzer.Events (Events)
import Scfuzzer.Types (Gas)
import Scfuzzer.Types.Campaign (WorkerState(..))
import Scfuzzer.Types.Config (Env(..))
import Scfuzzer.Types.Coverage (CoverageInfo)
import Scfuzzer.Types.Test qualified as T
import Scfuzzer.Types.Test (ScfuzzerTest(..))
import Scfuzzer.Types.Tx (Tx(..), TxCall(..))
...
mapTest :: ScfuzzerTest -> Test
mapTest test =
  let (status, transactions, err) = mapTestState test.state test.reproducer
  in Test
    { contract = "" -- TODO add when mapping is available https://github.com/khulnasoft-lab/scfuzzer/issues/415
