...
import Scfuzzer.ABI (encodeSig)
import Scfuzzer.Types (fromEVM)
import Scfuzzer.Types.Tx (TxCall(..), Tx(..), makeSingleTx, createTxWithValue, unlimitedGasPerBlock)
import Data.Set (Set)

-- | During initialization we can either call a function or create an account or contract
data Etheno
  -- | Registers an address with the scfuzzer runtime
  = AccountCreated Addr
...
-- | address containing scfuzzer tests
