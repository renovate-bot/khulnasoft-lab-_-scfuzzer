...
module Scfuzzer.Campaign where
...
import Scfuzzer.Mutator.Corpus
import Scfuzzer.Shrink (shrinkTest)
import Scfuzzer.Test
import Scfuzzer.Transaction
import Scfuzzer.Types (Gas)
import Scfuzzer.Types.Buffer (forceBuf)
import Scfuzzer.Types.Campaign
import Scfuzzer.Types.Corpus (Corpus, corpusSize)
import Scfuzzer.Types.Coverage (scoveragePoints)
import Scfuzzer.Types.Config
import Scfuzzer.Types.Signature (makeBytecodeCache, FunctionName)
import Scfuzzer.Types.Test
import Scfuzzer.Types.Test qualified as Test
import Scfuzzer.Types.Tx (TxCall(..), Tx(..), call)
import Scfuzzer.Types.World (World)
import Scfuzzer.Utility (getTimestamp)
...
isSuccessful :: [ScfuzzerTest] -> Bool
isSuccessful =
...
runUpdate
  ...
  => (ScfuzzerTest -> m (Maybe ScfuzzerTest))
  -> m ()
runUpdate f = do
  testsRef <- asks (.testsRef)
  tests <- liftIO $ readIORef testsRef
