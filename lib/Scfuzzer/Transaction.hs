...
module Scfuzzer.Transaction where
...
import Scfuzzer.Orphans.JSON ()
import Scfuzzer.Types (fromEVM)
import Scfuzzer.Types.Buffer (forceBuf, forceLit)
  ...
  let
    ...
    possibilities =
      ...
      , do value' <- lower tx'.value
           pure tx' { Scfuzzer.Types.Tx.value = value' }
      , do gasprice' <- lower tx'.gasprice
           pure tx' { Scfuzzer.Types.Tx.gasprice = gasprice' }
