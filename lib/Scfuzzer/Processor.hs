...
module Scfuzzer.Processor where
...
import Scfuzzer.ABI (hashSig, makeNumAbiValues, makeArrayAbiValues)
import Scfuzzer.Types.Signature (ContractName, FunctionName)
...
runSlither fp solConf = do
  ...
  let args = ["--ignore-compile", "--print", "scfuzzer", "--json", "-"]
             ++ solConf.khulnasoftArgs ++ [fp]
