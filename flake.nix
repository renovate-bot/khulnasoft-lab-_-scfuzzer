  ...
  outputs = { self, nixpkgs, flake-utils, nix-bundle-exe, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # TODO: maybe only override it for scfuzzer-redistributable?
        ...
        scfuzzer = pkgs.haskell.lib.dontCheck (
          with pkgs; lib.pipe
          (haskellPackages.callCabal2nix "scfuzzer" ./. { inherit hevm; })
          [
        ...
        scfuzzer-static = with pkgs; lib.pipe
          scfuzzer
          [
        ...
        then pkgs.runCommand "scfuzzer-stripNixRefs" {} ''
          mkdir -p $out/bin
          cp ${pkgs.haskell.lib.dontCheck scfuzzer-static}/bin/scfuzzer $out/bin/
          # fix TERMINFO path in ncurses
          ${perl} -i -pe 's#(${ncurses-static}/share/terminfo)#"/usr/share/terminfo" . "\x0" x (length($1) - 19)#e' $out/bin/scfuzzer
          chmod 555 $out/bin/scfuzzer
        '' else pkgs.runCommand "scfuzzer-stripNixRefs" {} ''
          mkdir -p $out/bin
          cp ${pkgs.haskell.lib.dontCheck scfuzzer-static}/bin/scfuzzer $out/bin/
          # get the list of dynamic libs from otool and tidy the output
          libs=$(${otool} -L $out/bin/scfuzzer | tail -n +2 | sed 's/^[[:space:]]*//' | cut -d' ' -f1)
          # get the path for libcxx
          cxx=$(echo "$libs" | ${grep} '^/nix/store/.*-libcxx')
          # rewrite /nix/... library paths to point to /usr/lib
          chmod 777 $out/bin/scfuzzer
          ${install_name_tool} -change "$cxx" /usr/lib/libc++.1.dylib $out/bin/scfuzzer
          # fix TERMINFO path in ncurses
          ${perl} -i -pe 's#(${ncurses-static}/share/terminfo)#"/usr/share/terminfo" . "\x0" x (length($1) - 19)#e' $out/bin/scfuzzer
          # re-sign binary
          CODESIGN_ALLOCATE=${codesign_allocate} ${codesign} -f -s - $out/bin/scfuzzer
          chmod 555 $out/bin/scfuzzer
        '';
      ...
      in rec {
        packages.scfuzzer = scfuzzer;
        packages.default = scfuzzer;

        packages.scfuzzer-redistributable = scfuzzerRedistributable;
